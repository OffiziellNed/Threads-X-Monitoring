...
export async function GET(request) {
  try {
    ...
    let rawItems = []; let allTitles = []; const puanKeywords = ['puan', 'maharani']; const now = new Date();
    for (let i = 1; i < items.length; i++) {
      ...
      if (titleMatch && dateMatch) {
        ...
        let rawTitle = titleMatch[1].replace(/<!\[CDATA\[(.*?)\]\]>/g, '$1').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#39;/g, "'"); const cleanTitle = rawTitle.split(" - ")[0];
    ...
    if (mode === 'terkini') {
        ...
        rawItems.forEach((item, index) => { const mainKeyword = item.topik.substring(0, 20).toLowerCase(); if (!seenTopics.has(mainKeyword)) { seenTopics.add(mainKeyword); dynamicIssues.push({ id: `puan-${index}`,...item, volume: 0 }); } });
    } else {
        rawItems.forEach((item, index) => { const volumeData = getRealVolume(item.topik, allTitles); const mainKeyword = item.topik.substring(0, 15).toLowerCase(); if (!seenTopics.has(mainKeyword)) { seenTopics.add(mainKeyword); dynamicIssues.push({ id: `puan-${index}`,...item, volume: volumeData }); } });
        dynamicIssues.sort((a, b) => b.volume - a.volume);
    }
    if (dynamicIssues.length === 0) { dynamicIssues.push({ id: "puan-empty", topik: `Tidak ada berita Puan Maharani dalam ${hours} jam terakhir.`, kategori: "Politik", volume: 0, source: "Sistem", pubDate: "Saat ini", articleTitle: "Radar Sepi", articleDesc: "Coba cek mode 12 jam.", link: "#", sourcesList: [] }); }
