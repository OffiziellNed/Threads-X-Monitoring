...
export async function GET(request) {
  try {
    ...
    let rawItems = []; let allTitles = []; const pdipKeywords = ['pdip', 'pdi perjuangan', 'pdi-p', 'megawati', 'hasto', 'ganjar', 'pramono', 'puan']; const now = new Date();
    ...
    if (mode === 'terkini') {
        ...
        rawItems.forEach((item, index) => { const mainKeyword = item.topik.substring(0, 20).toLowerCase(); if (!seenTopics.has(mainKeyword)) { seenTopics.add(mainKeyword); dynamicIssues.push({ id: `pdip-${index}`,...item, volume: 0 }); } });
    } else {
        rawItems.forEach((item, index) => { const volumeData = getRealVolume(item.topik, allTitles); const mainKeyword = item.topik.substring(0, 15).toLowerCase(); if (!seenTopics.has(mainKeyword)) { seenTopics.add(mainKeyword); dynamicIssues.push({ id: `pdip-${index}`,...item, volume: volumeData }); } });
        dynamicIssues.sort((a, b) => b.volume - a.volume);
    }
    if (dynamicIssues.length === 0) { dynamicIssues.push({ id: "pdip-empty", topik: `Tidak ada berita PDI Perjuangan dalam ${hours} jam terakhir.`, kategori: "Politik", volume: 0, source: "Sistem", pubDate: "Saat ini", articleTitle: "Radar Sepi", articleDesc: "Tidak ada pemberitaan.", link: "#", sourcesList: [] }); }
