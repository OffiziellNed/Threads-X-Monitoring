...
// =========================================================================
// FUNGSI PEMBERSIH URL (Anti Google Redirect & Ekstrak Link Asli)
// =========================================================================
  ...
  try {
    ...
    for (let i = 1; i < items.length; i++) {
      ...
      if (titleMatch && dateMatch) {
        ...
        // PEMBERSIH NAMA SUMBER: Memotong teks panjang setelah strip, koma, atau pipa[cite: 2]
        let rawSource = sourceMatch ? sourceMatch[1] : "Media Nasional";
        let cleanSource = rawSource.split(" - ")[0].split(",")[0].split("|")[0].trim();

        // EKSTRAK LINK ASLI DAN FORMAT TANGGAL[cite: 2]
        ...
        rawItems.push({
          ...
          link: linkAsli, // <-- Link asli yang langsung tembus ke sumber berita[cite: 2]
    ...
    if (dynamicIssues.length === 0) {
      dynamicIssues.push({ id: "mega-empty", topik: `Tidak ada berita Megawati Soekarnoputri dalam ${hours} jam terakhir.`, kategori: "Politik", volume: 0, source: "Sistem", pubDate: "Saat ini", articleTitle: "Radar Sepi", articleDesc: "Tidak ada pemberitaan.", link: "#", sourcesList: [] });
    }
