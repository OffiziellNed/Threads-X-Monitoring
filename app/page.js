...
import { ArrowLeft, TrendingUp, RefreshCw, ExternalLink, Calendar, Building2, Link as LinkIcon, Filter, DownloadCloud, Copy, CheckCircle2, Flag } from "lucide-react";
...
export default function SocialMediaMonitoring() {
  ...
  const handleOpenDetail = (isu) => {
    ...
    setScrapedResult(""); 
    setIsCopied(false);
    setPreviousPage(currentPage); // Menyimpan histori halaman
  ...
  // Format Nama Halaman secara Dinamis
  const getPageTitle = () => {
  ...
  // Apakah sedang mode PDIP? (Untuk merubah warna aksen)
    ...
    return (
      <main className="min-h-screen p-8 bg-[#0d1117] text-gray-200 font-sans flex flex-col items-center">
        <div className="w-full max-w-4xl space-y-8 mt-10">
          ...
          {/* SEKSI KHUSUS PDI PERJUANGAN */}
          <div className="space-y-4 pt-6">
            <div className="border-b border-red-900/50 pb-2 space-y-1">
              <h2 className="text-2xl font-bold text-red-500 flex items-center gap-2">
                <Flag size={24} /> PDI Perjuangan
              </h2>
              <p className="text-gray-400 text-sm">Monitoring berita dari PDI Perjuangan</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
  ...
  // --- HALAMAN MONITORING (BERLAKU UNTUK UMUM DAN PDIP) ---
  return (
    <main className="min-h-screen p-8 bg-[#0d1117] text-gray-200 font-sans flex flex-col items-center">
