...
import { ArrowLeft, TrendingUp, RefreshCw, ExternalLink, Calendar, Building2, Link as LinkIcon, Filter, DownloadCloud, Copy, CheckCircle2, Newspaper, Instagram } from "lucide-react";
...
export default function SocialMediaMonitoring() {
  ...
  // STATE SUMBER DATA: 'google' atau 'instagram'
  ...
  const fetchLiveTrends = async () => {
    ...
    try {
      ...
      // Fetch berdasarkan sumber data yang dipilih
      const response = await fetch(`/api/news?hours=${hours}&source=${dataSource}`);
  ...
  // Efek ini jalan tiap ganti halaman (3 jam / 12 jam) ATAU tiap switch sumber data (Google / IG)
  useEffect(() => {
    ...
    return (
      <main className="min-h-screen p-8 bg-[#0d1117] text-gray-200 font-sans flex flex-col items-center">
        <div className="w-full max-w-3xl space-y-6 mt-6">
          <button 
            onClick={() => setCurrentPage(currentPage === "detail" ? "12jam" : currentPage)} 
          ...
          <div className="bg-[#161b22] p-8 rounded-2xl shadow-xl border border-[#30363d] space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm text-gray-400 border-y border-[#30363d] py-4">
              <span className={`flex items-center gap-2 font-medium ${dataSource === 'instagram' ? 'text-pink-500' : 'text-blue-400'}`}>
                {dataSource === 'instagram' ? <Instagram size={16} /> : <Building2 size={16} />} 
                Sumber: {selectedIssue.source}
              </span>
            ...
            <div className="space-y-3 pt-2">
              <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-2">
                <LinkIcon size={16} className="text-blue-400" /> Tautan Terkait:
              </h3>
              <div className="grid grid-cols-1 gap-2.5">
                {selectedIssue.sourcesList.map((src, idx) => (
                  <a key={idx} href={src.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between bg-[#0d1117] hover:bg-[#1c2128] p-4 rounded-xl border border-[#30363d] text-gray-200 hover:text-blue-400 transition-all group shadow-sm">
                    <span className="font-medium text-sm flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-blue-500"></span>{src.name}
                    </span>
            ...
            <div className="pt-6 border-t border-[#30363d] space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <button 
                  ...
                  disabled={isScraping}
                  className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl font-medium shadow-md"
                >
        ...
        <div className="w-full max-w-4xl space-y-8 mt-10">
          ...
          <div className="bg-[#161b22] p-6 rounded-2xl shadow-lg border border-[#30363d] space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold flex items-center gap-2 text-white"><TrendingUp size={20} className="text-blue-400" /> Status Live Feed</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <button onClick={() => setCurrentPage("3jam")} className="p-6 bg-[#161b22] border border-[#30363d] rounded-2xl shadow-lg hover:border-blue-500 text-left space-y-2 group">
              ...
              <p className="text-gray-400 text-sm">Analisis lonjakan isu kilat.</p>
            </button>
            <button onClick={() => setCurrentPage("12jam")} className="p-6 bg-[#161b22] border border-[#30363d] rounded-2xl shadow-lg hover:border-blue-500 text-left space-y-2 group">
              ...
              <p className="text-gray-400 text-sm">Akumulasi tren berita 12 jam.</p>
            </button>
          </div>
        </div>
      </main>
    ...
    <main className="min-h-screen p-8 bg-[#0d1117] text-gray-200 font-sans flex flex-col items-center">
      <div className="w-full max-w-5xl space-y-6 mt-4">
        <div className="flex justify-between items-center">
          ...
          <button onClick={fetchLiveTrends} className="flex items-center gap-2 bg-[#161b22] text-blue-400 border border-[#30363d] px-4 py-2 rounded-xl text-sm hover:border-blue-500">
            <RefreshCw size={16} className={isLoading ? "animate-spin" : ""} /> Refresh Data
          </button>
        </div>

        {/* TOGGLE SUMBER DATA: GOOGLE VS INSTAGRAM */}
        <div className="flex flex-col sm:flex-row items-center justify-between bg-[#161b22] p-2 rounded-2xl border border-[#30363d]">
          <div className="flex w-full sm:w-auto">
            <button 
              onClick={() => setDataSource('google')}
              className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all ${
                dataSource === 'google' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:bg-[#1c2128]'
              }`}
            >
              <Newspaper size={18} /> Berita Nasional (Google)
            </button>
            <button 
              onClick={() => setDataSource('instagram')}
              className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all ${
                dataSource === 'instagram' ? 'bg-pink-600 text-white shadow-lg' : 'text-gray-400 hover:bg-[#1c2128]'
              }`}
            >
              <Instagram size={18} /> Opini Kreator (IG)
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {categories.map((cat) => (
            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-full text-sm font-medium border ${selectedCategory === cat ? 'bg-blue-600 text-white border-blue-500' : 'bg-[#161b22] text-gray-400 border-[#30363d] hover:bg-[#1c2128]'}`}>
              {cat}
            </button>
          ))}
        </div>
        ...
        {isLoading ? (
          ...
          <>
            <div className="bg-[#161b22] p-6 rounded-2xl shadow-lg border border-[#30363d] h-96">
              <h2 className="text-lg font-semibold mb-6 text-white">Grafik Top 5 {selectedCategory !== "Semua" && `(${selectedCategory})`} - {dataSource.toUpperCase()}</h2>
              {chartData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Bar dataKey="volume" fill={dataSource === 'instagram' ? '#ec4899' : '#3b82f6'} radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
            ...
            <div className="space-y-4 pb-10">
              ...
              {listData.length > 0 ? listData.map((isu, index) => (
                <div key={isu.id} className={`bg-[#161b22] p-6 rounded-2xl shadow-lg border border-[#30363d] border-l-4 hover:bg-[#1c2128] transition-colors ${dataSource === 'instagram' ? 'border-l-pink-500' : 'border-l-blue-500'}`}>
                  <div className="flex justify-between items-center">
                    <div className="pr-4">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <button onClick={() => handleOpenDetail(isu)} className={`text-white px-6 py-2.5 rounded-xl text-sm font-medium shadow-md ${dataSource === 'instagram' ? 'bg-pink-600 hover:bg-pink-500' : 'bg-blue-600 hover:bg-blue-500'}`}>
                        Buka
                      </button>
                    </div>
                  </div>
